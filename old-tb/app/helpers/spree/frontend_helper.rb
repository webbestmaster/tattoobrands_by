module Spree
  module FrontendHelper
    ...
    def breadcrumbs(taxon, separator="&nbsp;")
      ...
      crumbs = [content_tag(:li, content_tag(:span, link_to(content_tag(:span, "Каталог товаров", itemprop: "name"), spree.root_path, itemprop: "url") + separator, itemprop: "item"), itemscope: "itemscope", itemtype: "https://schema.org/ListItem", itemprop: "itemListElement")]
      if taxon
    ...
    def link_to_cart(text = nil)
      ...
      if simple_current_order.nil? or simple_current_order.item_count.zero?
        text = "<span class='header-basket-goods-cost'>Нет товаров</span>"
        css_class = 'empty'
      else
        text = "<span class='header-basket-goods-count'>#{simple_current_order.item_count}</span>"
        text += "<span class='header-basket-goods-cost js-usd-to-br-cart'>#{simple_current_order.display_total.to_html}</span>"
        css_class = 'full'
      end

      link_to text.html_safe, spree.cart_path, :class => "header-basket-wrapper #{css_class}"
    end
